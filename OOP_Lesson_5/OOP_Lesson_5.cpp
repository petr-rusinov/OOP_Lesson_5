...
//!!! С использованием const string& не сработает!!!
//! если использовать const string& в шаблоне
//! при передаче ссылки на строку
//!     StringValuePair<int> svp("Amazing", 7);
//! после этого конструктора временная строка убивается
//! и в следующем коде
//! std::cout << "Pair: " << svp.first() << ' ' << svp.second() << '\n';
//! "Amazing" не выведет, будет пустая строка
//! Чтобы сработал код, нужно именно копирование
